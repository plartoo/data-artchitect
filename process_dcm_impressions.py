...
def get_msg_body_for_completion(s3_export_folder, data_source_name):
    return """
        ...
        <br>
        Note: This Data Source is scheduled to run ETL automatically on Fridays at 5pm (EST), but if
...
def main():
    s3_folder = 'FilesForDatamart/DCMImpressions/'
    data_source_name = 'InCampaign DCM Impressions (DM 3128)'
    actions = [
                {'cmd': ['python', ROOT_FOLDER+'run_vsql.py', SQL_SCRIPT_FOLDER+'dcm_impressions_etl.sql']},
                {'cmd': ['python', ROOT_FOLDER + 'run_vsql_and_export_to_s3.py', SQL_SCRIPT_FOLDER + 'export_dcm_impressions.sql',
                         'DCMImpressions/', s3_folder, 'dcm_impressions'],
                 'notify_on_complete': {
                     'subject': 'Incampaign DCM Impressions data processed: please make sure that the Datamart completes the ETL process',
...
if __name__ == "__main__":
    ...
    print("This program runs processes every: Friday at 5pm EST")
